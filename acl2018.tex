...
In \cite{yousefi2017text} researchers present methods of extractive query-oriented single-document summarization using a deep auto-encoder (AE) to compute a feature space from the term-frequency (tf) input.
...
Researchers at IBM \cite{nallapati2016abstractive} modeled abstractive text summarization using Attentional Encoder-Decoder Recurrent Neural Networks, that was originally developed for machine translation to summarization and showed that this approach already outperformed state-of-the-art systems on two different English corpora.
...
In addition, they proposed a new dataset for multi sentence document summarization and established benchmark numbers on it.
